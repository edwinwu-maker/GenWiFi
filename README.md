...
本项目使用 MATLAB 生成 Wi-Fi 6 宽带业务场景和标准物理层波形对比实验。场景 1 模拟具有实际协议时序的 AP 下行业务；场景 2 复现 HE Extended-Range Single-User 单包实验；场景 3 生成 HE Multi-User OFDMA 波形。
...
运行 HE Extended-Range SU 单包对比场景：
...
时频图使用 2048 点周期 Hann 窗、1024 点重叠和 2048 点 NFFT，并将功率谱规整为 `1024 × 1024` 用于显示。
...
入口脚本在工作区返回 `txSUWaveform`、`txExtSUWaveform` 和 `scenarioInfo`，并绘制 HE-EXT-SU 频谱/时频图以及 L-STF/L-LTF 功率对比图。场景会验证扩展范围前导相对标准 HE-SU 增强约 3 dB，并检查 HE-Data 主要占用信道上半部。该单包实验不执行宽带重采样，也不写入 MAT 文件。
...
HE-MU 突发之间使用 DIFS 和随机退避，不生成 ACK。MATLAB R2023a 的 MAC 帧配置对象不支持 `HE-MU` 帧格式，因此场景按照波形生成文章的方式直接为每个用户生成随机 PSDU bit，而不模拟完整多用户 MAC 确认过程。
